...
var rgprice = parseInt(document.getElementById("rgprice").value);

setStatus("Initiating transaction... (please wait)");

metaset.produce( pid,pname,loc,disease,contact,doctor,rgprice, {from: account,gas:400000}).then(function() {
...
function fund(){
	...
	var receiver = parseInt(document.getElementById("pid").value);
...
metaget.getproduce.call( pid, {from: account}).then(function(value) {
...
metaset.test1( testno,group,charge,testdate,result, {from: account,gas:400000}).then(function() {
  setStatus("Transaction complete!");
...
var metaget = StructStorage.deployed();
var lid = document.getElementById("testnum").value;

setStatus("Initiating transaction... (please wait)");

metaget.gettest.call( lid,{from: account}).then(function(value) {
...
var metaget = StructStorage.deployed();
var pid = document.getElementById("getpid").value;
...
metaget.getproduce.call( pid,{from: account}).then(function(value) {
	...
	var cspan_element6 = document.getElementById("cgetval6");
  	var cstr = web3.toAscii(value[5]);
	cspan_element6.innerHTML = cstr;

	var cspan_element7 = document.getElementById("cgetval7");
	cspan_element7.innerHTML = value[6].valueOf();
  var cspan_element8 = document.getElementById("cgetval8");
    var cstr = parseInt(value[8]);
    cspan_element8.innerHTML = cstr;
